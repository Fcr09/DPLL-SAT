...
int DeleteUnit(problem* P, int unit, int depth);        //利用单子句规则化简P
void recovery(problem* P, int depth);                   //还原递归深度为depth时的修改操作
void AddUnit(problem* P, int unit);                     //首插法增加单子句
void deletelast(problem* P, int sup);                   //删除首插的单子句
int select(problem* P);                                 //最多变量选择策略
void sfree(st* s);                                      //释放子句
void pfree(problem* P);                                 //销毁问题
int solve(problem* P, int depth, int clue);             //递归求解函数
void Ansprint(FILE* fp, problem* P, int succ, int t);   //输出答案
int momselect(problem* P);                              //MOM策略
